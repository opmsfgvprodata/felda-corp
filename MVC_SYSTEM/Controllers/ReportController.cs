    ...
    public class ReportController : Controller
    {
        ...
        public ActionResult ActiveWorker(int? YearList, int WilayahIDList, int LadangIDList, int Status, string SyarikatList)
        {
            ...
            if (WilayahID == 0 && LadangID == 0)
            {
                ...
                //sarah added 20/10/2023
                SyarikatList2.Insert(0, (new SelectListItem { Text = GlobalResCorp.lblChoose, Value = "0" }));
                wlyhid = getwilyah.GetWilayahID(SyarikatID);
            ...
            else if (WilayahID != 0 && LadangID == 0)
            {
                ...
                //sarah added 23/10/2023
                WilayahIDList2.Insert(0, (new SelectListItem { Text = GlobalResCorp.lblChoose, Value = "0" }));
        ...
        public ActionResult GajiMinimaReport()
        {
            ...
            ViewBag.GajiMinima = new SelectList(
                db.tblOptionConfigsWeb.Where(x => x.fldOptConfFlag1 == "gajiMinima" && x.fldDeleted == false &&
                                            x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID),
        ...
        public ViewResult _TransactionListingRptSearch(int? WilayahList, int? LadangList, int? YearList, int? MonthList, string SyarikatList)
        {
        ...
        [AccessDeniedAuthorizeAttribute(Roles = "Super Power Admin,Super Admin,Admin 1,Admin 2,Admin 3, Super Power User, Viewer")]
        public ActionResult PermohonanIDBaruReport()
        {
            ViewBag.Report = "class = active";
            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
            int? getuserid = getidentity.ID(User.Identity.Name);
            ...
            statusList = new SelectList(
                db.tblOptionConfigsWeb
                    .Where(x => x.fldOptConfFlag1 == "statusapproval2" && x.fld_NegaraID == NegaraID &&
        ...
        [AccessDeniedAuthorizeAttribute(Roles = "Super Power Admin,Super Admin,Admin 1,Admin 2,Admin 3, Super Power User, Viewer")]
        public ViewResult _PermohonanIDBaruReport(int? WilayahList, int? LadangList, int? YearList, int? MonthList, int? StatusList, string print, string SyarikatList)
        {
            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
            int? getuserid = getidentity.ID(User.Identity.Name);
            GetNSWL.GetData(out NegaraID, out SyarikatID, out WilayahID, out LadangID, getuserid, User.Identity.Name);
            ...
            List<sp_RptPermohonanIDPenggunaBaruReport_Result> rptPermohonanIDPenggunaBaruResults = new List<sp_RptPermohonanIDPenggunaBaruReport_Result>();
            ...
            {
                List<sp_RptPermohonanIDPenggunaBaruReport_Result> resultreport = new List<sp_RptPermohonanIDPenggunaBaruReport_Result>();

                if (WilayahList == 0)
                {
                    dbSP.SetCommandTimeout(2600);
                    rptPermohonanIDPenggunaBaruResults = dbSP.sp_RptPermohonanIDPenggunaBaruReport(NegaraID, SyarikatID, WilayahList, LadangList, YearList, MonthList, StatusList, SyarikatList).ToList();
                }
                else
                {
                    if (LadangList == 0)
                ...
                if (rptPermohonanIDPenggunaBaruResults.Count == 0)
                {
                    ViewBag.Message = @GlobalResCorp.msgNoRecord;
                }
            }
            return View(rptPermohonanIDPenggunaBaruResults);
        }


        [AccessDeniedAuthorizeAttribute(Roles = "Super Power Admin,Super Admin,Admin 1,Admin 2,Admin 3, Super Power User, Viewer")]
        public ActionResult PermohonanPekerjaBaruReport()
        {
        ...
        //[ValidateAntiForgeryToken]
        [AccessDeniedAuthorizeAttribute(Roles = "Super Power Admin,Super Admin,Admin 1,Admin 2,Admin 3, Super Power User, Viewer")]
        public ViewResult _PermohonanPekerjaBaruReport(int? WilayahList, int? LadangList, int? YearList, int? MonthList, int? StatusList, string print, string SyarikatList)
        {
            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
            int? getuserid = getidentity.ID(User.Identity.Name);
            GetNSWL.GetData(out NegaraID, out SyarikatID, out WilayahID, out LadangID, getuserid, User.Identity.Name);
            ...
            List<sp_RptPermohonanPekerjaBru_Result> rptPermohonanPekerjaBruResults = new List<sp_RptPermohonanPekerjaBru_Result>();
            ...
            {
                List<sp_RptPermohonanPekerjaBru_Result> resultreport = new List<sp_RptPermohonanPekerjaBru_Result>();

                if (WilayahList == 0)
                {
                    dbSP.SetCommandTimeout(2600);
                    rptPermohonanPekerjaBruResults = dbSP.sp_RptPermohonanPekerjaBru(NegaraID, SyarikatID, WilayahList, LadangList, YearList, MonthList, StatusList, SyarikatList).ToList();
                }
                else
                {
                    if (LadangList == 0)
                ...
                if (rptPermohonanPekerjaBruResults.Count == 0)
                {
                    ViewBag.Message = @GlobalResCorp.msgNoRecord;
                }
            }
            return View(rptPermohonanPekerjaBruResults);
        }

        [AccessDeniedAuthorizeAttribute(Roles = "Super Power Admin,Super Admin,Admin 1,Admin 2,Admin 3, Super Power User, Viewer")]
        public ActionResult KedudukanPkjMengikutWlyhReport()
        {
            int[] wlyhid = new int[] { };
            int? NegaraID = 0;
            int? SyarikatID = 0;
            ...
            List<sp_RptKedPkjMenWlyh_Result> resultreport = new List<sp_RptKedPkjMenWlyh_Result>();

            return View("KedudukanPkjMengikutWlyhReport", resultreport);
        }
        ...
        public ActionResult KedudukanPkjMengikutWlyhReport(string SelectionDate, int WilayahIDList, int LadangIDList, string SyarikatList)
        {
            int[] wlyhid = new int[] { };
            int? NegaraID = 0;
            int? SyarikatID = 0;
            ...
            if (!string.IsNullOrEmpty(SelectionDate))
            {
                SelectedDate = DateTime.ParseExact(SelectionDate, "dd/MM/yyyy", null);
            }
            ...
            if (WilayahIDList == 0)
            {
                ...
                dbSP.SetCommandTimeout(600);
                resultreport = dbSP.sp_RptKedPkjMenWlyh(NegaraID, SyarikatID, WilayahIDList, LadangIDList, getuserid, SelectedDate, SyarikatList).ToList();
            }
            else
            {
                if (LadangIDList == 0)
        ...
        public ActionResult PocketCheckrollRpt()
        {
            ViewBag.Report = "class = active";
            //int month = timezone.gettimezone().AddMonths(-1).Month;
            ...
            var monthList = new SelectList(db.tblOptionConfigsWeb.Where(x => x.fldOptConfFlag1 == "monthlist" && x.fldDeleted == false && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID), "fldOptConfValue", "fldOptConfDesc", month);

            List<SelectListItem> StatusList = new List<SelectListItem>();
            StatusList = new SelectList(db.tblOptionConfigsWeb.Where(x => x.fldOptConfFlag1 == "statusaktif" && x.fldDeleted == false && x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID).OrderBy(o => o.fldOptConfValue).Select(s => new SelectListItem { Value = s.fldOptConfValue, Text = s.fldOptConfDesc }), "Value", "Text").ToList();
            StatusList.Insert(0, (new SelectListItem { Text = "Semua", Value = "0" }));

            List<SelectListItem> SelectionList = new List<SelectListItem>();
            SelectionList.Insert(0, (new SelectListItem { Text = "Sila Pilih", Value = "0" }));
            ...
            else if (WilayahID != 0 && LadangID == 0)
            {
                ...
                var wlyhid = getwilyah.GetWilayahID2(SyarikatID, WilayahID);
                WilayahList = new SelectList(db2.tbl_Wilayah.Where(x => wlyhid.Contains(x.fld_ID)), "fld_ID", "fld_WlyhName").ToList();
        ...
        public ActionResult _PocketCheckrollRptSearch(int? WilayahList, int? LadangList, int? RadioGroup, int? MonthList = 1, int? YearList = 2020, string SelectionList = "0", string StatusList = "0", string WorkCategoryList = "0", string print = "")
        {
            Connection Connection = new Connection();
            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
            int? getuserid = getidentity.ID(User.Identity.Name);
            ...
            var kerjaKesukaran = new List<ModelsEstate.tbl_KerjaKesukaran>();
            var kerjahdrCuti = new List<ModelsEstate.tbl_KerjahdrCuti>();
            var kerjaOT = new List<ModelsEstate.tbl_KerjaOT>();
            ...
            ViewBag.Kump = dbr.tbl_KumpulanKerja.Where(x => x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_LadangID == LadangID).ToList();
            ViewBag.NamaSyarikat = db5.tbl_Syarikat.Where(x => x.fld_SyarikatID == SyarikatID && x.fld_NegaraID == NegaraID && x.fld_Deleted == false).Select(s => s.fld_NamaSyarikat).FirstOrDefault();
            ViewBag.NoSyarikat = db5.tbl_Syarikat.Where(x => x.fld_SyarikatID == SyarikatID && x.fld_NegaraID == NegaraID && x.fld_Deleted == false).Select(s => s.fld_NoSyarikat).FirstOrDefault();
            ViewBag.NamaLadang = db5.tbl_Ladang.Where(x => x.fld_ID == LadangID && x.fld_Deleted == false).Select(s => s.fld_LdgCode + "-" + s.fld_LdgName).FirstOrDefault();
            ViewBag.activiti = db5.tbl_UpahAktiviti.Where(x => x.fld_SyarikatID == SyarikatID && x.fld_NegaraID == NegaraID && x.fld_Deleted == false).ToList();
            ViewBag.incentive = db5.tbl_JenisInsentif.Where(x => x.fld_SyarikatID == SyarikatID && x.fld_NegaraID == NegaraID && x.fld_Deleted == false).ToList();
            ViewBag.jenisCarumanTambahan = db5.tbl_CarumanTambahan.Where(x => x.fld_SyarikatID == SyarikatID && x.fld_NegaraID == NegaraID && x.fld_Deleted == false).ToList();
            ViewBag.pkjmast = pkjmast;
            ViewBag.kerjahdr = kerjahdr;
            ViewBag.kerja = kerja;
            ViewBag.kerjaKesukaran = kerjaKesukaran;
            ViewBag.kerjahdrCuti = kerjahdrCuti;
        ...
        //fatin added - 05/09/2023
        public JsonResult GetLadang3(int WilayahID)
        {
            List<SelectListItem> ladanglist = new List<SelectListItem>();
        ...
        public JsonResult GetList(int wlyh, int ldg, int RadioGroup, string StatusList)
        {
            ...
            return Json(new { SelectionList, SelectionLabel });
        }

        public JsonResult GetList2(int RadioGroup, int wlyh, int ladang, int cost)
        {
            Connection Connection = new Connection();
            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
            int? getuserid = getidentity.ID(User.Identity.Name);
            ...
            if (RadioGroup == 0)
            {
                ...
                if (ladang == 0)
                {
                    SelectionList = new SelectList(dbr.tbl_Pkjmast.Where(x => x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == wlyh).OrderBy(o => o.fld_Nama).Select(s => new SelectListItem { Value = s.fld_Nopkj, Text = s.fld_Nopkj + "-" + s.fld_Nama }), "Value", "Text").ToList();
                    SelectionList.Insert(0, (new SelectListItem { Text = "Semua", Value = "0" }));
                }
                else
                {
                    SelectionList = new SelectList(dbr.tbl_Pkjmast.Where(x => x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == wlyh && x.fld_LadangID == ladang).OrderBy(o => o.fld_Nama).Select(s => new SelectListItem { Value = s.fld_Nopkj, Text = s.fld_Nopkj + "-" + s.fld_Nama }), "Value", "Text").ToList();
                    SelectionList.Insert(0, (new SelectListItem { Text = "Semua", Value = "0" }));
                }
            ...
            else if (RadioGroup == 1)
            {
                ...
                {
                    SelectionList = new SelectList(dbr.vw_KumpulanKerja.Where(x => x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == wlyh && x.fld_deleted == false && x.fld_LadangID == ladang).OrderBy(o => o.fld_KodKumpulan).Select(s => new SelectListItem { Value = s.fld_KodKumpulan, Text = s.fld_KodKumpulan + "-" + s.fld_Keterangan }), "Value", "Text").ToList();
                    SelectionList.Insert(0, (new SelectListItem { Text = "Semua", Value = "0" }));
                }
            }
            else
            {
                ...
                SelectionList = new SelectList(db5.tbl_JenisInsentif.Where(x => x.fld_NegaraID == NegaraID && x.fld_SyarikatID == SyarikatID && x.fld_Deleted == false).OrderBy(o => o.fld_KodInsentif).Select(s => new SelectListItem { Value = s.fld_KodInsentif, Text = s.fld_KodInsentif + " - " + s.fld_Keterangan }), "Value", "Text").ToList();
                SelectionList.Insert(0, (new SelectListItem { Text = GlobalResCorp.lblAll, Value = "0" }));
            }
        ...
        //sarah added 23/10/2023
        public JsonResult GetLadang5(int WilayahID, string SyarikatID)
        {
            List<SelectListItem> ladanglist = new List<SelectListItem>();
            ...
            if (SyarikatID2 == 1)
            {
                if (getwilyah.GetAvailableWilayah(SyarikatID2))
                {
                    if (WilayahID == 0)
                    {
                        ladanglist = new SelectList(db2.vw_NSWL.Where(x => x.fld_NegaraID == NegaraID && x.fld_CostCentre == SyarikatID && x.fld_Deleted_L == false).OrderBy(o => o.fld_LdgCode).Select(s => new SelectListItem { Value = s.fld_LadangID.ToString(), Text = s.fld_LdgCode + " - " + s.fld_NamaLadang }), "Value", "Text").ToList();
                    }
                    else
                    {
                        ladanglist = new SelectList(db2.vw_NSWL.Where(x => x.fld_NegaraID == NegaraID && x.fld_CostCentre == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_Deleted_L == false).OrderBy(o => o.fld_LdgCode).Select(s => new SelectListItem { Value = s.fld_LadangID.ToString(), Text = s.fld_LdgCode + " - " + s.fld_NamaLadang }), "Value", "Text").ToList();
                    }
                }
            }
            ...
            if (SyarikatID2 == 2)
            {
                if (WilayahID == 0)
                {
                    ladanglist = new SelectList(db.vw_NSWL.Where(x => x.fld_SyarikatID.ToString() == SyarikatID && x.fld_Deleted_L == false).OrderBy(o => o.fld_NamaLadang).Select(s => new SelectListItem { Value = s.fld_LadangID.ToString(), Text = s.fld_LdgCode + " - " + s.fld_NamaLadang }), "Value", "Text").ToList();
                    //ladanglist.Insert(0, (new SelectListItem { Text = "Sila Pilih", Value = "0" }));
                }
                else
                {
                    ladanglist = new SelectList(db.vw_NSWL.Where(x => x.fld_SyarikatID.ToString() == SyarikatID && x.fld_WilayahID == WilayahID && x.fld_Deleted_L == false).OrderBy(o => o.fld_NamaLadang).Select(s => new SelectListItem { Value = s.fld_LadangID.ToString(), Text = s.fld_LdgCode + " - " + s.fld_NamaLadang }), "Value", "Text").ToList();
                    //ladanglist.Insert(0, (new SelectListItem { Text = "Sila Pilih", Value = "0" }));
                }
            }
        ...
        [AccessDeniedAuthorizeAttribute(Roles = "Super Power Admin,Super Admin,Admin 1,Admin 2,Admin 3, Super Power User, Viewer")]
        public ActionResult RumusanPekerjaBulanKeseluruhan()
        {
            ViewBag.Report = "class = active";
            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
            int? getuserid = getidentity.ID(User.Identity.Name);
            ...
            if (syarikatselection == 0 && WilayahID == 0 && LadangID == 0)
            {
                var syarikatInfo = db.tblOptionConfigsWeb.Where(x => x.fldOptConfFlag1 == "kodSAPSyarikat" && x.fldDeleted == false && x.fld_NegaraID == NegaraID).OrderBy(x => x.fldOptConfDesc).FirstOrDefault();
                int SyarikatCode = Convert.ToInt16(syarikatInfo.fld_SyarikatID);
                WilayahIDList.Insert(0, (new SelectListItem { Text = GlobalResReport.sltAll, Value = "0" }));
                LadangIDList.Insert(0, (new SelectListItem { Text = GlobalResReport.sltAll, Value = "0" }));
                SyarikatIDList = new SelectList(db.tblOptionConfigsWeb.Where(x => x.fldOptConfFlag1 == "kodSAPSyarikat" && x.fldDeleted == false && x.fld_NegaraID == NegaraID).OrderBy(o => o.fldOptConfDesc).Select(s => new SelectListItem { Value = s.fldOptConfValue, Text = s.fldOptConfDesc }), "Value", "Text").ToList();
                //SyarikatIDList.Insert(0, (new SelectListItem { Text = GlobalResReport.sltAll, Value = "0" }));
                incldg = 0;
            }
            else if (SyarikatID != 0 && WilayahID == 0 && LadangID == 0)
            {
                var syarikatInfo = db.tblOptionConfigsWeb.Where(x => x.fldOptConfFlag1 == "kodSAPSyarikat" && x.fldDeleted == false && x.fld_NegaraID == NegaraID).OrderBy(x => x.fldOptConfDesc).FirstOrDefault();
                int SyarikatCode = Convert.ToInt16(syarikatInfo.fld_SyarikatID);
                var listladang2 = db.tbl_Ladang.Where(x => x.fld_CostCentre == syarikatInfo.fldOptConfValue.ToString() && x.fld_SyarikatID == SyarikatCode && x.fld_Deleted == false).Select(x => x.fld_WlyhID).ToList();
        ...
        [AccessDeniedAuthorizeAttribute(Roles = "Super Power Admin,Super Admin,Admin 1,Admin 2,Admin 3, Super Power User, Viewer")]
        public ViewResult _RumusanPekerjaBulanKeseluruhan(int? WilayahList, int? LadangList, int? YearList, string print, string SyarikatList)
        {
            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
            int? getuserid = getidentity.ID(User.Identity.Name);
            GetNSWL.GetData(out NegaraID, out SyarikatID, out WilayahID, out LadangID, getuserid, User.Identity.Name);

            List<sp_RptMasterJumPkj_Result> rptMasterJumPkjResults = new List<sp_RptMasterJumPkj_Result>();
            ViewBag.NamaSyarikat = db.tbl_Syarikat
                .Where(x => x.fld_SyarikatID == SyarikatID && x.fld_NegaraID == NegaraID)
                .Select(s => s.fld_NamaSyarikat)
                .FirstOrDefault();
            ...
            try
            {
                ...
                {
                    rptMasterJumPkjResults = dbSP.sp_RptMasterJumPkj(NegaraID, SyarikatID, WilayahList, LadangList, YearList, getuserid, SyarikatList)
                        .ToList();

                    if (rptMasterJumPkjResults.Count == 0)
                    {
                        ViewBag.Message = @GlobalResCorp.msgNoRecord;
                    }
                }
            ...
            return View(rptMasterJumPkjResults);
        }
        ...
        [AccessDeniedAuthorizeAttribute(Roles = "Super Power Admin,Super Admin,Admin 1,Admin 2,Admin 3, Super Power User, Viewer")]
        public ActionResult RumusanPekerjaKontraktor()
        {
            ViewBag.Report = "class = active";
            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
            int? getuserid = getidentity.ID(User.Identity.Name);
        ...
        [AccessDeniedAuthorizeAttribute(Roles = "Super Power Admin,Super Admin,Admin 1,Admin 2,Admin 3, Super Power User, Viewer")]
        public ViewResult _RumusanPekerjaKontraktor(int? WilayahList, int? LadangList, int? YearList, string print, string SyarikatList)
        {
            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
            int? getuserid = getidentity.ID(User.Identity.Name);
            GetNSWL.GetData(out NegaraID, out SyarikatID, out WilayahID, out LadangID, getuserid, User.Identity.Name);

            List<sp_RptRumKedPkjKontraktor_Result> rptKedPkjKontraktorResults = new List<sp_RptRumKedPkjKontraktor_Result>();
            ViewBag.NamaSyarikat = db.tbl_Syarikat
                .Where(x => x.fld_SyarikatID == SyarikatID && x.fld_NegaraID == NegaraID)
                .Select(s => s.fld_NamaSyarikat)
                .FirstOrDefault();
            ...
            try
            {
                ...
                try
                {
                    ...
                    parameters.Add("Year", YearList);
                    parameters.Add("UserID", getuserid);
                    parameters.Add("CompCode", SyarikatList);
                    con.Open();
                    rptKedPkjKontraktorResults = SqlMapper.Query<sp_RptRumKedPkjKontraktor_Result>(con, "sp_RptRumKedPkjKontraktor", parameters, commandType: CommandType.StoredProcedure).ToList();

                    if (String.IsNullOrEmpty(WilayahList.ToString()) || String.IsNullOrEmpty(LadangList.ToString()))
                    {
                        ViewBag.Message = "Sila pilih tahun, syarikat, wilayah dan ladang";
                    }
                    ...
                    else
                    {
                        ...
                        if (rptKedPkjKontraktorResults.Count == 0)
                        {
                            ViewBag.Message = @GlobalResCorp.msgNoRecord;
                        }
                    }
                ...
                //else
                //{
                //    dbSP.SetCommandTimeout(12000);
                //    rptKedPkjKontraktorResults = dbSP.sp_RptRumKedPkjKontraktor(NegaraID, SyarikatID, WilayahList, LadangList, YearList, getuserid, SyarikatList)
                //        .ToList();
            ...
            return View(rptKedPkjKontraktorResults);
        }
        ...
        public ViewResult _ProductivityRptSearch(int? MonthList, int? YearList, string print, string SyarikatIDList, string WilayahIDList, string LadangIDList)
        {
            ...
            if (WilayahID == 0 && LadangID == 0)
            {
                listsyarikat = db.tblOptionConfigsWeb.Where(x => x.fldOptConfFlag1 == "kodSAPSyarikat" && x.fldDeleted == false && x.fld_NegaraID == NegaraID).OrderBy(x => x.fldOptConfDesc).ToList();
                ...
                LadangIDList2 = new SelectList(db.tbl_Ladang.Where(x => x.fld_CostCentre == SyarikatIDList.ToString() && x.fld_Deleted == false && x.fld_SyarikatID == SyarikatIDCode.fld_SyarikatID && x.fld_NegaraID == NegaraID && x.fld_WlyhID == WilayahID).OrderBy(x => x.fld_LdgName).Select(s => new SelectListItem { Value = s.fld_ID.ToString(), Text = s.fld_ID + "-" + s.fld_LdgName }), "Value", "Text", WilayahIDList).ToList();
                LadangIDList2.Insert(0, (new SelectListItem { Text = GlobalResReport.sltAll, Value = "0" }));
            ...
            else if (WilayahID != 0 && LadangID == 0)
            {
                ...
                LadangIDList2 = new SelectList(db2.tbl_Ladang.Where(x => x.fld_WlyhID == WilayahID && x.fld_Deleted == false && x.fld_CostCentre == SyarikatIDList.ToString()).OrderBy(o => o.fld_LdgName).Select(s => new SelectListItem { Value = s.fld_ID.ToString(), Text = s.fld_LdgCode + " - " + s.fld_LdgName }), "Value", "Text").ToList();
                LadangIDList2.Insert(0, (new SelectListItem { Text = GlobalResReport.sltAll, Value = "0" }));
                listsyarikat = db.tblOptionConfigsWeb.Where(x => x.fldOptConfFlag1 == "kodSAPSyarikat" && x.fldDeleted == false && x.fld_NegaraID == NegaraID).OrderBy(x => x.fldOptConfDesc).ToList();
                SyarikatIDList2 = new SelectList(listsyarikat.OrderBy(x => x.fldOptConfDesc).Select(s => new SelectListItem { Value = s.fldOptConfValue.ToString(), Text = s.fldOptConfDesc }), "Value", "Text", SyarikatIDList).ToList();
            ...
            List<ModelsDapper.sp_RptProduktiviti_Corp_Result> RptProduktiviti_Corp_Result = new List<ModelsDapper.sp_RptProduktiviti_Corp_Result>();
            ...
            else
            {
                DynamicParameters parameters = new DynamicParameters();
                parameters.Add("NegaraID", NegaraID);
                parameters.Add("SyarikatID", getSyarikatId);
                parameters.Add("WilayahID", WilayahIDList);
                parameters.Add("LadangID", LadangIDList);
                parameters.Add("Year", YearList);
                parameters.Add("Month", MonthList);
                parameters.Add("UserID", getuserid);
                parameters.Add("CostCentre", SyarikatIDList);
               ...
               public JsonResult GetPkj(int? WilayahList, int? LadangList)
        {
            ...
            try
            {
                ...
                foreach (var estateWorker in estateWorkerData)
                {
                    ...
                    {
                        Text = estateWorker.fld_Nopkj,
                        Value = estateWorker.fld_Nopkj
                    });
                }
            }
            catch (Exception ex)
        ...
        public ViewResult _WorkerIncentiveRptSearch(int? RadioGroup, int? MonthList, int? YearList,
              string SelectionList, string StatusList, string WorkCategoryList, string print, int? SyarikatIDList, int? WilayahIDList, int? LadangIDList, string statusList)
        {
            ...
            ViewBag.NoSyarikat = db.tbl_Syarikat
                ...
                .Select(s => s.fld_NoSyarikat)
                .FirstOrDefault();
            ViewBag.InfoLadang = db.tbl_Ladang
            ...
            if (String.IsNullOrEmpty(WilayahIDList.ToString()) || String.IsNullOrEmpty(SyarikatIDList.ToString()))
            {
                ViewBag.Message = @GlobalResCorp.msgNoRecord;
            }
            else
            {
                ...
                var workerData = estateConnection.vw_MaklumatInsentif
                    .Join(estateConnection.tbl_Pkjmast,
                        incentive => incentive.fld_Nopkj,
                        pkj => pkj.fld_Nopkj,
                        (incentive, pkj) => new { incentive, pkj })
                    .Where(x =>
                        //x.pkj.fld_Kdaktf == "1" &&
                        x.incentive.fld_SyarikatID == SyarikatID &&
                ...
                if (!String.IsNullOrEmpty(statusList?.ToString()))
                {
                    workerData = workerData.Where(x => x.pkj.fld_Kdaktf == statusList);
                }
                if (!String.IsNullOrEmpty(WilayahIDList?.ToString()))
                {
                    workerData = workerData.Where(x => x.pkj.fld_WilayahID == WilayahIDList);
                }

                if (!String.IsNullOrEmpty(LadangIDList?.ToString()) && LadangIDList != 0)
                {
                    workerData = workerData.Where(x => x.pkj.fld_LadangID == LadangIDList);
                }

                if (RadioGroup == 0 && SelectionList != "0")
                {
                    workerData = workerData.Where(x => x.pkj.fld_Nopkj == SelectionList);

                }

                if (RadioGroup == 1 && SelectionList != "0")
                {
                    var groupData = estateConnection.tbl_KumpulanKerja
                    .Where(x => x.fld_KodKumpulan == SelectionList && x.fld_NegaraID == NegaraID &&
                                x.fld_SyarikatID == SyarikatID && x.fld_WilayahID == WilayahIDList &&
                                x.fld_LadangID == LadangIDList)
                    .Select(s => s.fld_KumpulanID)
                    .SingleOrDefault();
                    workerData = workerData.Where(x => x.pkj.fld_KumpulanID == groupData);
                }

                if (RadioGroup == 2 && SelectionList != "0")
                {
                    workerData = workerData.Where(x => x.incentive.fld_KodInsentif == SelectionList);
                }

                var workerDataList = workerData.ToList();
                
                var ladangData = db.tbl_Ladang
                    .Where(l => l.fld_CostCentre == SyarikatIDList.ToString())
                    .ToList();

                var combinedData = from estate in workerDataList
                                   join ladang in ladangData on estate.pkj.fld_LadangID equals ladang.fld_ID
                                   where ladang.fld_CostCentre == SyarikatIDList.ToString()
                                   orderby ladang.fld_LdgName, estate.pkj.fld_Nama
                                   select new { estate.pkj, estate.incentive, ladang };

                var result = combinedData
                    .GroupBy(x => new { x.pkj.fld_Nopkj })
                    .Select(g => g.First())
                    .OrderBy(x => x.incentive.fld_KodInsentif)
                    .ToList();

                if (RadioGroup == 2) {
                    foreach (var i in combinedData)
                    {
                        MaklumatInsentifPekerja.Add(new vw_MaklumatInsentifPekerja
                        {
                            Pkjmast = i.pkj,
                            insentif = i.incentive,
                            ladang = db.tbl_Ladang.Where(item => item.fld_ID == i.pkj.fld_LadangID).First()
                        });
                    }
                }
                else
                {
                    ...
                    {
                        var pendapatan = combinedData.Where(a => a.incentive.fld_Nopkj == (i.incentive.fld_Nopkj) && a.incentive.fld_KodInsentif.Contains("P") )
                                      //.OrderBy(x => x.incentive.fld_KodInsentif)
                                       .Select(x => x.incentive)
                                      .ToList();

                        var potongan = combinedData
                            .Where(a => a.incentive.fld_Nopkj == (i.incentive.fld_Nopkj) && a.incentive.fld_KodInsentif.Contains("T"))
                            //.OrderBy(x => x.incentive.fld_KodInsentif)
                            .Select(x => x.incentive)
                            .ToList();
