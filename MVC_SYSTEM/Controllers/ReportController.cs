...
namespace MVC_SYSTEM.Controllers
{
    ...
    public class ReportController : Controller
    {
        ...
        [AccessDeniedAuthorizeAttribute(Roles = "Super Power Admin,Super Admin,Admin 1,Admin 2,Admin 3, Super Power User, Viewer")]
        public ActionResult ReportKekuatanPekerjaLadang()
        {
            ViewBag.Report = "class = active";
            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
            int? getuserid = getidentity.ID(User.Identity.Name);
        ...
        [AccessDeniedAuthorizeAttribute(Roles = "Super Power Admin,Super Admin,Admin 1,Admin 2,Admin 3, Super Power User, Viewer")]
        public ViewResult _ReportKekuatanPekerjaLadang(int? WilayahList, int? LadangList, string print, string SyarikatList)
        {
            int? NegaraID, SyarikatID, WilayahID, LadangID = 0;
            int? getuserid = getidentity.ID(User.Identity.Name);
            GetNSWL.GetData(out NegaraID, out SyarikatID, out WilayahID, out LadangID, getuserid, User.Identity.Name);

            List<sp_RptKekuatanPkjLdg_Result> rptKekuatanPkjResults = new List<sp_RptKekuatanPkjLdg_Result>();
            ViewBag.NamaSyarikat = db.tbl_Syarikat
                .Where(x => x.fld_SyarikatID == SyarikatID && x.fld_NegaraID == NegaraID)
                .Select(s => s.fld_NamaSyarikat)
                .FirstOrDefault();
            ...
            try
            {
                ...
                try
                {
                    ...
                    con.Open();
                    rptKekuatanPkjResults = SqlMapper.Query<sp_RptKekuatanPkjLdg_Result>(con, "sp_RptKekuatanPkjLdg", parameters, commandType: CommandType.StoredProcedure).ToList();

                    if (String.IsNullOrEmpty(WilayahList.ToString()) || String.IsNullOrEmpty(LadangList.ToString()))
                    {
                        ViewBag.Message = "Sila pilih syarikat, wilayah dan ladang";
                    }
                    ...
                    else
                    {
                        ...
                        if (rptKekuatanPkjResults.Count == 0)
                        {
                            ViewBag.Message = @GlobalResCorp.msgNoRecord;
                        }
                    }
            ...
            return View(rptKekuatanPkjResults);
        }
